...
- **Digital Agency GIF** (CC0 1.0) and **自治体標準オープンデータセット** (公共データ利用規約 PDL1.0, the successor of the 推奨データセット since 2023-03-31): the Japanese source of truth for addresses and municipal datasets. Mapped field by field, never copied as a competing model. The IMI 共通語彙基盤, which GIF succeeded, is not a mapping target.
...
- Code: Apache-2.0. Model content: CC BY 4.0, which anything derived from Smart Data Models requires. GIF content (CC0 1.0) is re-licensed under CC BY 4.0; 自治体標準オープンデータセット material is used under the 公共データ利用規約 (PDL1.0, compatible with CC BY 4.0) and credited to its source.
