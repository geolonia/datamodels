...
const T = {
  ja: {
    ...
    required: '必須', indexed: 'インデックス', pii: '個人情報', value: '値', relationshipTo: '→', license: 'このページのモデル内容は CC BY 4.0 で提供されています。',
  ...
  en: {
    ...
    required: 'required', indexed: 'indexed', pii: 'personal data', value: 'Value', relationshipTo: '→', license: 'Model content on this page is licensed under CC BY 4.0.',
...
function modelPage(lang, prefix, subject, model) {
  ...
  md += `# ${model.type} ${statusBadge(lang, model.catalog.status ?? 'draft')}\n\n`;
  md += `**${title}** <span style="color:var(--vp-c-text-2)">/ ${model.catalog.title?.[other] ?? ''}</span>\n\n${desc}\n\n`;
  md += `| | |\n|---|---|\n`;
  md += `| ${t.typeIri} | ${code(mu.typeIri)} |\n`;
  ...
  md += `| ${t.examples} | [key-values](${rel(mu.examples)}example.json) · [normalized](${rel(mu.examples)}example-normalized.jsonld) |\n`;
  md += `| ${t.geonicdb} | [${code(rel(mu.geonicdb))}](${rel(mu.geonicdb)}) ${t.geonicdbNote} |\n`;
  ...
  md += `## ${t.attributes} {#attributes}\n\n`;
  for (const [name, prop] of attributesOf(model)) {
    const flags = [required.has(name) ? badge('warning', t.required) : '', prop['x-geonicdb']?.indexed ? badge('info', t.indexed) : '', prop['x-geonicdb']?.pii ? badge('danger', t.pii) : ''].filter(Boolean).join(' ');
    md += `### ${name} {#${name}}\n\n`;
    if (flags) md += `${flags}\n\n`;
    md += `${model.catalog.attributes?.[name]?.[lang] ?? prop.description ?? ''}\n\n`;
    md += `- ${t.value}: ${valueText(lang, subject, prop, prefix)}\n- IRI: ${code(prop['x-iri'] ?? '')}\n\n`;
  }
...
function subjectPage(lang, prefix, subject) {
  ...
  md += `## ${t.models} {#models}\n\n| Type | | |\n|---|---|---|\n`;
  for (const m of subject.models) md += `| [${m.type}](${prefix}${rel(modelUrls(subject, m).page)}) | ${m.catalog.title?.[lang] ?? ''} | ${statusBadge(lang, m.catalog.status ?? 'draft')} |\n`;
...
export async function generateSitePages(subjects) {
  for (const [lang, prefix] of [['ja', ''], ['en', '/en']]) {
    ...
    for (const s of subjects) {
      await put(join(base, s.name, 'index.md'), subjectPage(lang, prefix, s));
