# IncidentHandoverNote

申し送り / Incident handover note

One handover note about an incident report, from one team to another.

通報に対する申し送り1件。送信元・送信先の班と内容を持つ。

- Type IRI: `https://models.geonicdb.com/ns/disaster/IncidentHandoverNote`
- Context: `https://models.geonicdb.com/context/disaster/v1.jsonld` (alias), `https://models.geonicdb.com/context/disaster/v1.1.0.jsonld` (exact)
- Schema: `https://models.geonicdb.com/schema/disaster/IncidentHandoverNote/v1.json`
- Page: https://models.geonicdb.com/models/disaster/IncidentHandoverNote/

Files follow the Smart Data Models layout: `schema.json` (key-values representation, with `x-ngsi`, `x-iri` and `x-geonicdb` annotations), `catalog.yaml` (Japanese and English descriptions), `examples/`, `notes.yaml`, `ADOPTERS.yaml`.
